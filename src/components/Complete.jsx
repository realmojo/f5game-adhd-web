...
export const Complete = () => {
  ...
  useEffect(() => {
    ...
    if (0 <= sum && sum <= 36) {
      ...
      setName("양호 단계");
      setText(
        "현대 사회인들은 누구나 한 가지씩 사소한 실수를 할 때도 있습니다. 하지만 이는 보편적으로 누구에게나 나타나는 증상이며 당신은 ADHD가 아닙니다. 다만 심신을 안정시키는 스트레칭이나 가벼운 산책을 하면서 삶을 유지하면 더 좋은 감정상태를 만드실 수 있습니다."
      );
    } else if (73 <= sum && sum <= 108) {
      setName("정상 단계");
      setText(
        "당신은 다소 집중력이 부족할 수 있고 충동적인 행동을 간혹 할 수도 있으나 본인의 의지로 충분히 잘 해나갈 수 있습니다. 아직까지는 ADHD라고 보기에는 어렵습니다. 하지만 조심해야 할 단계 입니다. 따듯한 차를 마시면서 심신을 가라앉히는 행동을 추천드립니다."
      );
    } else if (109 <= sum && sum <= 144) {
      setName("경고 단계");
      setText(
        "ADHD로 가기 마지막 단계이며 음주가무에 빠져서 건강과 사회생활 및 전반적인 삶을 살아가기 어려울 수 있습니다. 주변을 조금씩 돌아보고 충동적이나 감정훈련 조절 및 취미새활 같은 것을 즐기면서 삶을 살다보면 자연스레 좋아지는 경우가 많습니다."
      );
    } else if (145 <= sum && sum <= 180) {
      setName("ADHD 매우 높음");
      setText(
        "당신은 ADHD일 확률이 매우 높습니다. 이 질환을 오래가지고 계시면 기분장애나 충동억제등을 조절하기가 매우 어렵습니다. 더 나아가 우울장애를 일으켜 내 주변의 사람들까지 괴로울 수 있습니다. 가까운 정신과 병원에 들려서 자세한 상담을 받아보시기를 추천드립니다."
      );
    }
